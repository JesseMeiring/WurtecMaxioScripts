...
function showHideCarStations() {
	for(let i = 0; i < carStationDesignationFields.length; i++){
		showHideCustomVariableField(carStationDesignationFields[i], SaaSField.val() > i)
...
function showHideLobbyStations() {
	let shown = true;
	for(let i = 0; i < lobbyStationDesignnationFields.length; i++){
		showHideCustomVariableField(lobbyStationDesignnationFields[i], i == 0 || shown)
		showHideCustomVariableField(lobbyStationSerialFields[i], i == 0 || shown)
		if(shown) shown = (lobbyStationDesignnationFields[i].val() != "" || lobbyStationSerialFields[i].val() != "")
	}
}
...
function showHideMonitoring(){
	for(let i = 0; i < monitoringServiceFields.length; i++){
		showHideCustomVariableField(monitoringServiceFields[i], WurtecWatchField.val() == 0)
	}
}
